//! Graph Topo WASM agent.
//! Handles topological sort requests — returns RESULT with intent=45 (TOPO).
#![no_std]
use core::slice;
...
const INTENT_OUT: u8  = 45; // TOPO intent
...
pub unsafe extern "C" fn handle_block(ptr: *const u8, len: usize) -> *mut u8 {
    ...
    let (_, _, ch, pri, conf, _pref) = sw_decode(word);
    // payload_ref = 0 for topo (no specific start node)
    let result_word = sw_encode(TYPE_RESULT, INTENT_OUT, ch, pri,
                                conf.saturating_add(500).min(0xFFFF), 0);
